...
use self::consumer::{
    SlugCounts, classify_embed, classify_import, classify_link, html_heading_text_span,
    scan_bare_paths_in_text, strip_code_spans,
};

// The scanner's, the tree builder's and the consumer helpers' public entry
// points keep their `crate::block::…` spelling: they are what the inline
// parser, the structural scanners, the formatter and the move engine call into,
// and the split is meant to be invisible to them.
pub use self::consumer::{
    byte_offset_to_line, classify_media, external_namespace, extract_html_heading_text,
    github_slug, gitlab_slug, normalize_path, vscode_slug,
};
