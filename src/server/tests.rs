...
fn type_hierarchy_supertypes_returns_parent() {
    ...
    let parents = type_hierarchy_supertypes(&workspaces, &h3_item).expect("should return parents");
...
fn type_hierarchy_subtypes_returns_children() {
    ...
    let children = type_hierarchy_subtypes(&workspaces, &h1_item).expect("should return children");
...
fn type_hierarchy_h1_has_no_supertypes() {
    ...
    let h1_item = heading_to_hierarchy_item(&h1, &dir.path().join("a.md"));
    let parents = type_hierarchy_supertypes(&workspaces, &h1_item).expect("should return empty");
...
fn definition_on_body_link_stays_on_target_document() {
    ...
    let loc = go_to_definition(&workspaces, &params).expect("definition should resolve the target");
    assert!(
        loc.uri.ends_with("b.md"),
...
fn completion_predicate_skipped_on_exempt_links() {
    ...
    let external = complete_at(&workspaces, &dir, "ext.md", 0, 25).expect("known title context");
    assert!(
...
fn completion_reference_label_offers_definitions() {
    let dir = workspace_with_files(&[("doc.md", "[def]: https://example.com/page\n\nSee [link][")]);
    let workspaces = scan_workspaces(&dir);
...
fn tokens_for(workspaces: &Workspaces, dir: &tempfile::TempDir, rel: &str) -> Vec<DecodedToken> {
...
fn folder_add_over_open_rootless_document_keeps_buffer_and_leaves_no_orphan() {
    ...
    assert!(
        doc.data.tree.source().contains("In Buffer") && !doc.data.tree.source().contains("On Disk"),
...
fn nested_folder_without_marker_is_entry_point_into_covering_scope() {
    ...
    assert!(
        !diags.iter().any(
            |d| d.file.as_path() == Path::new("top.md") && d.message.contains("does not exist")
        ),
...
fn host_steering_into_the_submodule_survives_and_names_the_submodule() {
    ...
    let steering = diagnostics
        ...
        .unwrap_or_else(|| panic!("the host's link into the submodule errors: {diagnostics:?}"));
    assert!(
...
fn mid_session_config_typo_holds_last_good_and_flags_the_config_uri() {
    ...
    let valid_config = "[[override]]\npaths = [\"report.md\"]\nstale_references = { expect = 2 }\n";
    let report_text = "See `gone-a.md` and `gone-b.md` here.\n";
    let dir = workspace_with_files(&[(".lattice.toml", valid_config), ("report.md", report_text)]);
    let mut workspaces = scan_workspaces(&dir);
    let report_uri = file_uri(&dir, "report.md");
...
fn drift_and_unused_override_warnings_publish_on_the_config_uri() {
    ...
    let mut buf = Vec::new();
    let _failed = crate::lint::run(dir.path(), false, true, false, &mut buf).expect("lint runs");
...
fn config_watcher_event_applies_under_an_open_dirty_config_draft() {
    ...
    let dir = workspace_with_files(&[(".lattice.toml", ""), ("doc.md", "See `gone.md` here.\n")]);
    let mut workspaces = scan_workspaces(&dir);
    let config_uri = file_uri(&dir, ".lattice.toml");
    let doc_uri = file_uri(&dir, "doc.md");
...
fn config_did_save_commits_and_the_watcher_echo_is_a_no_op() {
    ...
    fs::write(dir.path().join(".lattice.toml"), new_config).expect("write the new config on disk");
    let (server, client) = Connection::memory();
    drive(
        &server,
        &mut workspaces,
