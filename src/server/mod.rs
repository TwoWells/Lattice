...
// Re-exported so `crate::server::collect_all_diagnostics` and
// `crate::server::merge_perspectives` keep naming these two wherever the
// submodules put them: the shared invariants harness ([`crate::invariants`])
// asserts against both, and it is the only caller outside this module — hence
// the same `cfg` it is compiled under, so the re-export is never dead weight in
// a release build.
